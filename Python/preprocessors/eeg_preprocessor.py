...
if __name__ == "__main__":
    ...
    usage = lambda: print("[PREPROC] Usage: python eeg_preprocessor.py <input_file> [l_freq] [h_freq] [reference]") or sys.exit(1)
    ...
    run = lambda input_file, l_freq, h_freq, reference: (
        ...
        print(f"[PREPROC] EEG preprocessing started for participant: {input_file}") or (
            (lambda ext: (
                (lambda raw: (
                    (lambda _: (
                        (lambda _: (
                            pl.DataFrame({ch: raw.get_data(picks=[ch])[0] for ch in raw.ch_names}).write_parquet(get_output_filename(input_file)),
                            print(f"[PREPROC] EEG preprocessing finished for file: {input_file}")
                    ...
                    print(f"[PREPROC] EEG preprocessing errored for file: {input_file}. NaNs detected in EEG data."),
                ...
                (lambda df: (
                        ...
                        (lambda raw: (
                            (lambda _: (
                                ...
                                (lambda standardized_df: (
                                    ...
                                    print(f"[PREPROC] EEG preprocessing finished for file: {input_file} (FIF + parquet)")
                                ))(pl.DataFrame({
                    ...
                    print(f"[PREPROC] EEG preprocessing errored for file: {input_file}. Unsupported file type: {ext}"),
                    pl.DataFrame([]).write_parquet(get_output_filename(input_file)),
                    sys.exit(1)
                )
            ...
            print(f"[PREPROC] EEG preprocessing errored for file: {input_file}. Missing filter frequencies."),
            pl.DataFrame([]).write_parquet(get_output_filename(input_file)),
            sys.exit(1)
        )
    ...
    try:
        ...
        print(f"[PREPROC] EEG preprocessing errored for file: {sys.argv[1] if len(sys.argv) > 1 else 'unknown'}. Error: {e}")
        sys.exit(1)