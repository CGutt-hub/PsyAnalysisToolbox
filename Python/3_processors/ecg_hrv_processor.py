        ...
        if len(nn_intervals_ms) >= 2: # Need at least one NNI, which means at least 2 R-peaks
            ...
            if len(nn_interval_times_sec) == len(nn_intervals_ms) and \
               ...
               (len(nn_interval_times_sec) <= 1 or np.all(np.diff(nn_interval_times_sec) > 0)):
                ...
                if num_nni_points >= 2: # interp1d needs at least 2 points for linear
                    interp_kind = 'linear'
                    ...
                    interp_func = interp1d(nn_interval_times_sec, nn_intervals_ms, kind=interp_kind, fill_value="extrapolate", bounds_error=False)

                    min_time_interp = nn_interval_times_sec[0]
                    max_time_interp = nn_interval_times_sec[-1]

                    if max_time_interp > min_time_interp:
                        ...
                        self.logger.warning(f"ECGHRVProcessor - P:{participant_id}: Max time not greater than min time for NNI interpolation.")
                else: # num_nni_points < 2
                    self.logger.warning(f"ECGHRVProcessor - P:{participant_id}: Less than 2 NN intervals ({num_nni_points}). Cannot interpolate for continuous HRV.")
            else:
