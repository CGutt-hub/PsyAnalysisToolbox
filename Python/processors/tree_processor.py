...
def preprocess_run(ip, entry_delim, depth_key, kv_delim):
    ...
    print(f"[PROC] Processing {len(entries)} entries")
    ...
    print(f"[PROC] Built tree: root with {len(root.children)} children")
    ...
    if root.children:
        ...
        if first_entity:
            ...
            if not has_level:
                ...
                print(f"[PROC] Added Level: 1 property to first entity")
    ...
    print(f"[PROC] Found levels: {sorted(nodes_by_level.keys())}, max={max_level}")
    ...
    for level in range(1, max_level + 1):
        ...
        if any(get_onset(c) == float('inf') for c in children):
            print(f"[PROC] Skipping Level {level}: {len(children)} children (no timestamps)")
            continue
        ...
        if not parents_have_onset:
            ...
            print(f"[PROC] Level {level}: sibling sort ({len(children)} children)")
            for p in parents:
            ...
            print(f"[PROC] Level {level}: temporal reparenting ({len(children)} children)")
    ...
    print(f"[PROC] Globally reordered {max_level} levels by temporal onset")
    ...
    return out
if __name__ == '__main__': (lambda a: preprocess_run(a[1], a[2], a[3], a[4]) if len(a) == 5 else (print("[PREPROC] Usage: python tree_processor.py <input.parquet> <entry_delim> <depth_delim> <kv_delim>"), sys.exit(1)))(sys.argv)