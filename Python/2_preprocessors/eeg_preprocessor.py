...
from typing import Optional, Tuple, List, Union, cast # Import cast

# Module-level defaults for EEGPreprocessor
...
class EEGPreprocessor:
    ...
    def process(self,
        ...
        if eeg_filter_band_config is None or not (isinstance(eeg_filter_band_config, (list, tuple)) and len(eeg_filter_band_config) == 2):
            self.logger.error("EEGPreprocessor - 'eeg_filter_band_config' must be a list/tuple of two numbers. Skipping.")
            return None
        # ica_n_components_config can be None (MNE default), int, float, or 'rank'.
        # ica_random_state_config can be None (MNE default).
        if ica_accept_labels_config is None or not isinstance(ica_accept_labels_config, list): # Empty list is valid
            self.logger.error("EEGPreprocessor - 'ica_accept_labels_config' must be a list. Skipping.")
            return None
        if ica_reject_threshold_config is None or not isinstance(ica_reject_threshold_config, (int, float)):
            self.logger.error("EEGPreprocessor - 'ica_reject_threshold_config' must be a number. Skipping.")
            return None

        # Determine final config values, using defaults if not provided or invalid
        final_eeg_ref = DEFAULT_EEG_REFERENCE
        if eeg_reference_config is not None:
            if isinstance(eeg_reference_config, str) and eeg_reference_config.strip():
                ...
                self.logger.warning(f"EEGPreprocessor: Invalid 'eeg_reference_config' ('{eeg_reference_config}'). Using default: '{DEFAULT_EEG_REFERENCE}'.")

        final_eeg_ref_proj = DEFAULT_EEG_REFERENCE_PROJECTION
        if eeg_reference_projection_config is not None:
            if isinstance(eeg_reference_projection_config, bool):
                ...
                self.logger.warning(f"EEGPreprocessor: Invalid 'eeg_reference_projection_config' ('{eeg_reference_projection_config}'). Using default: '{DEFAULT_EEG_REFERENCE_PROJECTION}'.")

        final_fir_design = DEFAULT_FILTER_FIR_DESIGN
        if filter_fir_design_config is not None:
            if isinstance(filter_fir_design_config, str) and filter_fir_design_config.strip():
                ...
                self.logger.warning(f"EEGPreprocessor: Invalid 'filter_fir_design_config' ('{filter_fir_design_config}'). Using default: '{DEFAULT_FILTER_FIR_DESIGN}'.")
        ...
        if ica_max_iter_config is None:
            final_ica_max_iter = DEFAULT_ICA_MAX_ITER
            ...
            self.logger.warning(f"EEGPreprocessor: Invalid 'ica_max_iter_config' ('{ica_max_iter_config}'). Using default: '{DEFAULT_ICA_MAX_ITER}'.")
            final_ica_max_iter = DEFAULT_ICA_MAX_ITER

        final_ica_label_method = DEFAULT_ICA_LABELING_METHOD
        if ica_labeling_method_config is not None:
            if isinstance(ica_labeling_method_config, str) and ica_labeling_method_config.strip():
                ...
                self.logger.warning(f"EEGPreprocessor: Invalid 'ica_labeling_method_config' ('{ica_labeling_method_config}'). Using default: '{DEFAULT_ICA_LABELING_METHOD}'.")
        ...
        try:
            ...
            # Determine the value to pass to max_iter.
            # The preceding logic ensures final_ica_max_iter is either 'auto' (str) or an int.
            # Use cast for the int case to help type checkers, though type: ignore might still be needed
            # if stubs are severely incorrect.
            max_iter_value: Union[int, str]
            if final_ica_max_iter == 'auto':
                max_iter_value = final_ica_max_iter # This is 'auto' (str)
            elif isinstance(final_ica_max_iter, int):
                max_iter_value = cast(int, final_ica_max_iter) # This is an int, cast for checker

            # Instantiate ICA using the determined value. Use type: ignore if checker flags max_iter.
            ica_instance = mne.preprocessing.ICA(n_components=ica_n_components_config,
                                        random_state=ica_random_state_config,
                                        max_iter=max_iter_value) # MNE API accepts int or 'auto'.
            ...
            try:
                ...
                # Ensure ica_accept_labels_config contains strings for comparison
                safe_accept_labels = [str(lbl) for lbl in ica_accept_labels_config]
                
                exclude_idx = [ # Indices of components to exclude
                    idx for idx, label in enumerate(labels)
                    if str(label) not in safe_accept_labels and
